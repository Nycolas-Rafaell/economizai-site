...
function updateMetrics() {
  ...
  document.getElementById('cardsCounter').textContent = `${offers.length} ${offers.length === 1 ? 'card cadastrado' : 'cards cadastrados'}`;
}
...
function render() {
  ...
  const visible = offers.filter((offer) => !term || `${offer.title || ''} ${offer.category || ''} ${offer.subcategory || ''}`.toLocaleLowerCase('pt-BR').includes(term));
  cards.replaceChildren(...visible.map((offer) => {
    ...
    row.innerHTML = `<div class="admin-card-media">${image}<span class="store-badge">${marketplaceLabel(offer)}</span><span class="offer-status">${labels[state]}</span></div><div class="admin-card-body"><p class="admin-card-category">${escapeHtml(category)}</p><div class="admin-card-title">${escapeHtml(offer.title)}</div><div class="admin-card-rating">${ratingText}</div><div class="admin-price-block">${oldPrice}<div class="admin-price-current"><small>Por</small><strong>${money(offer.currentPrice)}</strong>${discount ? `<span class="admin-discount">-${discount}% OFF</span>` : ''}</div></div><div class="card-actions"><a href="admin.html?cadastro=1&editar=${encodeURIComponent(offer.id)}">Editar</a><button class="delete" type="button">Excluir</button></div></div>`;
    row.querySelector('.delete').addEventListener('click', () => removeOffer(offer, row.querySelector('.delete')));
    return row;
  }));
  hint.textContent = visible.length ? '' : (term ? 'Nenhum card encontrado para essa busca.' : 'Nenhum card cadastrado ainda.');
}
